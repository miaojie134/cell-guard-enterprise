          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center p-3 bg-blue-50 rounded-lg">
                      ...
                      <p className="text-xl font-bold text-blue-600">{batchStatus.totalEmployeesToProcess}</p>
                      <p className="text-xs text-gray-600">目标员工数</p>
                    </div>
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      <CheckCircle className="h-6 w-6 mx-auto mb-2 text-green-600" />
                      <p className="text-xl font-bold text-green-600">{batchStatus.tokensGeneratedCount}</p>
                      <p className="text-xs text-gray-600">令牌生成数</p>
                    </div>
                    <div className="text-center p-3 bg-purple-50 rounded-lg">
                      <Mail className="h-6 w-6 mx-auto mb-2 text-purple-600" />
                      <p className="text-xl font-bold text-purple-600">{batchStatus.emailsAttemptedCount}</p>
                      <p className="text-xs text-gray-600">邮件尝试数</p>
                    </div>
                    <div className="text-center p-3 bg-orange-50 rounded-lg">
                      <AlertCircle className="h-6 w-6 mx-auto mb-2 text-orange-600" />
                      <p className="text-xl font-bold text-orange-600">{batchStatus.emailsFailedCount}</p>
                      <p className="text-xs text-gray-600">邮件失败数</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="space-y-4">
                ...
                <CardContent className="space-y-3">
                  <div>
                    <div className="flex items-center mb-1">
                      <ScopeIcon className="h-4 w-4 mr-1 text-gray-600" />
                      <span className="text-sm font-medium text-gray-500">盘点范围</span>
                    </div>
                    <p className="text-sm">{scopeDisplay.text}</p>
                  </div>
                  {scopeDisplay.detail && (
                    <div>
                      <span className="text-sm font-medium text-gray-500">范围详情</span>
                      <p className="text-sm text-gray-700 break-all">{scopeDisplay.detail}</p>
                    </div>
                  )}
                  <div>
                    <span className="text-sm font-medium text-gray-500">有效期</span>
                    <p className="text-sm">{batchStatus.requestedDurationDays} 天</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-500">创建时间</span>
                    <p className="text-sm">{new Date(batchStatus.createdAt).toLocaleString('zh-CN')}</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600">{emailSuccessRate}%</p>
                    <p className="text-xs text-gray-600">发送成功率</p>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>成功发送</span>
                      <span className="text-green-600">{batchStatus.emailsSucceededCount}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>发送失败</span>
                      <span className="text-red-600">{batchStatus.emailsFailedCount}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>总计尝试</span>
                      <span>{batchStatus.emailsAttemptedCount}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              {batchStatus.status === VERIFICATION_STATUS.COMPLETED_WITH_ERRORS && (
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-orange-600 text-lg">处理建议</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <Alert className="py-2">
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription className="text-sm">
                        部分邮件发送失败，建议检查失败的员工邮箱地址是否正确，或手动联系相关员工。
                      </AlertDescription>
                    </Alert>
                  </CardContent>
                </Card>
